...
//
    ...
    sprintf(statefile_name, "dump/state%d.csv", id);
    ...
    if (gpu && params.sbparams.A0 != 0){
        ...
        for (int i = 0; i < params.max_dumps; ++i){
            ...
            if(params.alg.compare("onesite") == 0){
                ...
                for (int n = 0; n < params.eq_time/params.max_dumps; ++n){
                    if(wolff.step_one_site_prealloc_lowmem(latref, &fast_action, temp_corr, thrustsite_trash_ref,
                        thrustint_site_trash_ref,site_corr_trash_ref,thrustlat_trash_ref,thrustint_trash_ref,full_corr_trash_ref,
                        &full_forward_plan, &full_backward_plan, &onesite_forward_plan, &onesite_backward_plan,
                        cuda_status, full_state_rs, full_state_ft, onesite_state_rs, onesite_state_ft)){
        ...
        for (int dump = 0; dump < params.max_dumps; ++dump){
            ...
            //write to files
            timer.flag_start_time("file write");
            file.open(dumpfile_name,std::ofstream::out | std::ofstream::trunc);
            results.write_results(&file, "vals");
            file.close();

            file.open(corrfile_name,std::ofstream::out | std::ofstream::trunc);
            file << lat.get_Lx() << "," << lat.get_Ly() << "\n";
            results.write_results(&file, "corr");
            file.close();

            file.open(vortcorrfile_name,std::ofstream::out | std::ofstream::trunc);
            file << lat.get_Lx() << "," << lat.get_Ly() << "\n";
            results.write_results(&file, "vort_corr");
            file.close();

            file.open(statefile_name,std::ofstream::out | std::ofstream::trunc);
            file << lat.get_Lx() << " " << lat.get_Ly() << "\n";
            results.write_results(&file, "states");
            file.close();
            timer.flag_end_time("file write");
...
//
    ...
    std::cout << "Test results: functions.size() = " << results.functions.size() << ", functions[0].size() = " << results.functions[0].size() << ", functions[0][0].size() = " << results.functions[0][0].size() << "\n";
	file.open(dumpfile_name,std::ofstream::out | std::ofstream::trunc);
    results.write_results(&file, "vals");
    file.close();
