...
//
    std::cout << "Spin Boson MC Simulation Started\n";
...
//
    ...
    results.functions = {{}};
    double loc, xmag, mag, ptemp_moves = 0, ptemp_total = 0;
    ...
    if (gpu && params.sbparams.A0 != 0){
        ...
        lat.get_thrust_vector(thrustlat_ref);
        //fast_action = cufft_calc_action(thrustlat_ref, thrustint_ref, temp_corr, latref.get_Lx(), latref.get_Ly());//lazy implementation of cufft_calc_action
        ...
        for (int i = 0; i < params.max_dumps; ++i){
            ...
            if(params.alg.compare("onesite") == 0){
                for (int j = 0; j < params.eq_time/params.max_dumps; ++j){
                    num_accept += wolff.step_one_site_prealloc(latref, &fast_action, temp_corr,
...
//
	char dumpfile_name[100], corrfile_name[100], statefile_name[100];
...
//
    if(id == 0){
        ...
        std::vector<double> locs(p), loc2s(p), loc4s(p), loc_binds(p), loc_bind_errs(p), 
                            ...
                            actions(p), sxs(p), xkinkss(p), stagger_mags(p), clusters(p);
        ind_vars[0] = ind_var;
	    ...
	    fprintf(file, "%-10.10s %-10.10s %-10.10s %-10.10s %-10.10s %-10.10s %-10.10s %-10.10s %-10.10s %-10.10s %-10.10s %-10.10s\n", 
                vary_param, "loc2", "xmag2", "mag2", "loc_bind", "xmag_bind", "mag_bind", "sx", "xkinks", "xstagger", "action", "cluster");
        for (int i = 0; i < p; ++i){
            fprintf(file, "%-10.6f %-10.6f %-10.6f %-10.6f %-10.6f %-10.6f %-10.6f %-10.6f %-10.6f %-10.6f %-10.6f %-10.6f\n", 
                    ...
                    actions[i]/params.lengths[0]/params.lengths[1], clusters[i]/params.lengths[0]/params.lengths[1]);
        }
        fclose(file);
